...
// in-place sigmoid on the raw gate logits L[B*Q*F] (contiguous, 16B-aligned) -> SL, 8 bf16/thread.
// vectorized via int4 (8 bf16 = 16 bytes). Replaces the sigmoid part of sigpad with NO Fp expansion.
__global__ void k_sigmoid_inplace(__nv_bfloat16* L, long n8) {
    for (long c = (long)blockIdx.x * blockDim.x + threadIdx.x; c < n8; c += (long)gridDim.x * blockDim.x) {
        __nv_bfloat16* p = L + c * 8;
        int4 v = *reinterpret_cast<int4*>(p);
        __nv_bfloat16* h = reinterpret_cast<__nv_bfloat16*>(&v);
        #pragma unroll
        for (int j = 0; j < 8; j++) h[j] = __float2bfloat16(1.f / (1.f + __expf(-b2f(h[j]))));
...
// L row stride F (=1491), SL row stride Fp. The 293M exp now live in the gate epilogue (free) -> this
// pass is pure memory (~0.35 ms vs ~0.7-0.9 ms for the exp+pad sigpad).
__global__ void k_repad(const __nv_bfloat16* __restrict__ L, __nv_bfloat16* __restrict__ SL,
    ...
    for (long row = blockIdx.x; row < nrows; row += gridDim.x) {
        const __nv_bfloat16* lr = L + row * F;
        __nv_bfloat16* sr = SL + row * Fp;
        for (int f = threadIdx.x; f < F; f += blockDim.x)   // pad [F,Fp) pre-zeroed at alloc; copy only valid F
            sr[f] = lr[f];
    }
}
...
extern "C" int kernel_run(__nv_bfloat16** inputs, int num_inputs,
    ...
    // stats + fused LayerNorm -> M[B,F] (column-major), R[B,F], N_pad[B,Fp,D]  (one X read)
    int warps_per_blk = 8;
    ...
    if (getenv("GIST_SKIP_GATE") == nullptr) {
        int rc = run_gate(g_M, P, g_L, B, F, QF, stream);   // CUTLASS WGMMA gate, sigmoid fused in epilogue (M col-major, K=F)
        if (rc != 0) return rc;
    }
